...
export interface QuestionVerificationProps {
  affirmation: string
  options: string[]
...
export function QuestionVerification({
  affirmation,
  options,
  onAnswer,
  disabled = false,
  correctIndex,
}: QuestionVerificationProps) {
  ...
  return (
    <div className="rounded-xl border border-ink/10 bg-white p-6 shadow-sm">
      <p className="mb-2 font-body text-sm font-medium text-ink/60">
        Quelle est la meilleure question à poser pour vérifier cette affirmation ?
      </p>

      <p className="mb-6 font-body italic leading-relaxed text-ink/80">"{affirmation}"</p>
      ...
      <div className="space-y-3">
        {options.map((option, i) => (
          <button
            ...
            aria-label={`Question ${i + 1}: ${option}${revealed && i === correctIndex ? " (correcte)" : ""}`}
      ...
      {revealed && correctIndex !== undefined && (
        ...
        >
          {selected === correctIndex
            ? "Bonne question de vérification !"
            : `Pas tout à fait. La meilleure question était : "${options[correctIndex]}".`}
        </p>
      )}
    </div>
  )
